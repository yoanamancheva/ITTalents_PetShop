...
public class CartController extends BaseController {
    ...
    private boolean sessionContainsProduct(long id, HttpSession session) throws BaseException {
        ...
        while ((attributes.hasMoreElements())) {
            ...
            if(attribute.equals(product.getName() + "_product")) {
                return true;
            }
        }
        return false;
    ...
    public Object addProductsToCart(@PathVariable("id") long id, @PathVariable("quantity") int quantity, HttpSession session) throws BaseException{
        validateLogin(session);
        Product product = productController.getById(id, session);
        if(sessionContainsProduct(id, session) && product.getQuantity() >= quantity && product.getQuantity() > 0) {
            int previousNumber = (int)session.getAttribute(product.getName() + "_product");
            session.setAttribute(product.getName() + "_product", previousNumber + quantity);
            showCart(session);
            ...
            session.setAttribute(product.getName() + "_product", quantity);
            showCart(session);
    ...
    public Object removeProductsFromCart(@PathVariable("id") long id, @PathVariable("quantity") int quantity, HttpSession session) throws BaseException{
        validateLogin(session);
        Product product = productController.getById(id, session);
        String attribute = product.getName()+ "_product";
        ...
        if(sessionContainsProduct(id, session) &&  (int)session.getAttribute(attribute) >= quantity && quantity > 0) {
            ...
            session.setAttribute(product.getName() + "_product", previousNumber - quantity);
            showCart(session);
            return new ErrorMsg("You successfully removed " +quantity +" " + product.getName() + " from your cart", LocalDateTime.now(), HttpStatus.OK.value());
        }
        else {
